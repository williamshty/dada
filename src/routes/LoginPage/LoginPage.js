...
class LoginPage extends React.Component {
  ...
  async loadLoginVerificationCode() {
    const verification_code = await getLoginVerificationCode(
      localStorage.getItem("phoneNum")
    );
    console.log(verification_code);
    if (
      verification_code.data.code !== 1001 &&
      verification_code.data.code !== 1002
    ) {
  ...
  submitLoginForm() {
    if (
      this.state.enteredVerification !== this.state.receivedVerification ||
      this.state.enteredVerification === ""
    ) {
  ...
  render() {
    return (
      <div className={styles.base__container}>
        ...
        <div className={styles.status__bar__filler} />
        <div
        ...
        <div className={styles.login__text}>验证登录</div>
          ...
          if (this.state.verificationError === "true") {
            return (
              <div>
                ...
                <div className={styles.error__text}>验证码输入错误</div>`
              </div>
            );
          }
        })()}
